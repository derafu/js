...
describe('__.tabs() with <a> nav links', () => {
  let baseUrl;

  beforeAll(async () => {
    const absolutePath = path.join(__dirname, '../html/__.tabs-a.test.html');
  ...
  // Verify that a specific card is highlighted when clicking on a magic link.
  test('Mark a specific card when clicking on a magic link', async () => {
    ...
    const isCardMarked = await page.evaluate(() => {
      ...
      return card.classList.contains('deeplink-highlight') && card.classList.contains('border-primary');
    });
    expect(isCardMarked).toBe(true);
    expect(page.url()).toContain('#tab1:card2');
  });

  // Verify that a specific input container is highlighted when clicking on its label.
  test('Mark a specific input when clicking on its label', async () => {
    ...
    const isInputMarked = await page.evaluate(() => {
      ...
      const container = el.closest('.input-group') || el.closest('.form-floating') || el.parentElement;
      return container.classList.contains('deeplink-highlight');
    });
    expect(isInputMarked).toBe(true);
    expect(page.url()).toContain('#tab3:input1');
  });

  // Verify that a specific select container is highlighted when clicking on its label.
  test('Mark a specific select when clicking on its label', async () => {
    ...
    const isSelectMarked = await page.evaluate(() => {
      const el = document.querySelector('#select1');
      const container = el.closest('.input-group') || el.closest('.form-floating') || el.parentElement;
      return container.classList.contains('deeplink-highlight');
    });
  ...
  // Verify that a specific select2 container is highlighted when clicking on its label.
  test('Mark a specific select2 when clicking on its label', async () => {
    await page.click('#tab3-tab');
    await page.click('label[for="select2"]');
    const isSelectMarked = await page.evaluate(() => {
      const el = document.querySelector('#select2');
      const container = el.closest('.input-group') || el.closest('.form-floating') || el.parentElement;
      return container.classList.contains('deeplink-highlight');
    });
    expect(isSelectMarked).toBe(true);
  ...
  // Verify that a specific textarea container is highlighted when clicking on its label.
  test('Mark a specific textarea when clicking on its label', async () => {
    ...
    const isTextareaMarked = await page.evaluate(() => {
      const el = document.querySelector('#textarea1');
      const container = el.closest('.input-group') || el.closest('.form-floating') || el.parentElement;
      return container.classList.contains('deeplink-highlight');
    });
